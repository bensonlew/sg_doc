...
功能描述
--------
类似sam格式的文件/sequence ID文件中提取序列信息，从原始的fastq文件中提取或剔除相应信息的序列，得到新的fastq
...
2.当传入"seq_id_list"参数时，必须同时传入"fastq_dir"和"extract_type"参数，确定需要删除/提取的序列ID;
 当传入"sam"参数时，直接从sam文件中提出mapping和unmapping 的序列存成对应的fq文件。

3.最终生成生成相关fq文件，且内部会统计相应的reads数和碱基数等信息。
