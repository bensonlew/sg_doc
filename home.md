...
* [mongo片键设计原则](#user-content-mongo片键设计原则)
...
# mongo片键设计原则
在设计一个collection片键的时候，一定要对collection的结构有足够的了解，明白这个collection常用的一些查询操作逻辑，知道其索引。同时在选择片键的时候要充分考虑到数据插入的时候能够均匀插入到不同分片上，避免热分片出现。
1. 片键的字段一定要是索引，但是索引不一定要设置为片键；
2. 片键一般为经常查询的字段；
3. 片键为连续型字段的时候（例如日期或者递增序列），需要设置为hashed哈希类型；
4. 选择能够再分的片键，第一片键查询记录较多时，为了防止巨大块的出现，就需要设置第二片键进行辅助分片；
5. 同一时间的读写要均匀分布到各个分片中；
6. 选择可用于普遍查询的片键，在单次的查询和更新时能够定位到单一分片；
7. 特例：mongo自带的文件存储集合fs.files，是不需要建片键的。</br>
附上片键文档的格式：

| 集合名称 |  片键    |
| :------------- | :------------- |
| sg_pt_qc       |  {sample_id: hashed,  _id:1}      |
|fs.chunks|{file_id:1, n:1}
